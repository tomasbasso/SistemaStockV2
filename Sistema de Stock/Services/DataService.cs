...
namespace Sistema_de_Stock.Services
{
    ...
    /// Ruta automáticamente entre Supabase PostgreSQL (online) y SQLite cache (offline).
    /// Las operaciones de escritura requieren conexión a internet.
    /// </summary>
    public class DataService
    {
        private readonly StockOnlineContext _onlineDb;
        private readonly StockCacheContext  _cacheDb;
        private readonly ConnectivityService _connectivity;

        // Contexto activo según conectividad — tipado como IStockContext para acceder a los DbSets
        private IStockContext _db => _isTestMode ? _testDb : (_connectivity.IsOnline ? _onlineDb : _cacheDb);

        // Acceso directo a DbContext para operaciones que requieren Database/ChangeTracker
        private DbContext _dbContext => _isTestMode ? (DbContext)_testDb : (_connectivity.IsOnline ? (DbContext)_onlineDb : (DbContext)_cacheDb);


        private readonly IStockContext _testDb;
        private readonly bool _isTestMode;

        public DataService(StockOnlineContext onlineDb, StockCacheContext cacheDb, ConnectivityService connectivity)
        {
            _onlineDb    = onlineDb;
            _cacheDb     = cacheDb;
        ...
        public DataService(IStockContext db)
        {
            _testDb = db;
            _isTestMode = true;
        }
        ...
        /// <summary>
        /// Lanza excepción si no hay internet. Llamar al inicio de todos los métodos de escritura.
        /// </summary>
        private void EnsureOnline()
        {
            ...
            if (!_connectivity.IsOnline)
                throw new InvalidOperationException(
                    "Sin conexión a internet. El sistema está en modo solo lectura.");
        }
        ...
        /// <summary>
        /// Inicializa la base de datos cache offline.
        /// La DB online (Supabase) no necesita inicialización local.
        /// </summary>
        public async Task InitializeAsync()
        {
            await _cacheDb.InitializeCacheAsync();
        }
        ...
        public async Task<double> CalcularRotacionAnualAsync()
        {
            try 
            {
                ...
                // Unidades vendidas (vía LINQ para que se aplique el Global Query Filter de TenantId)
                var unidadesVendidas = await _db.VentaDetalles
                    .Where(vd => vd.UpdatedAt >= haceUnAnio) // Aproximación por fecha de detalle o mejor aún, relacionando con Venta
                    .Join(_db.Ventas.Where(v => !v.IsDeleted),
                        vd => vd.VentaId,
                        v => v.Id,
                        (vd, v) => new { vd.Quantity, v.Date })
                    .Where(x => x.Date >= haceUnAnio)
                    .SumAsync(x => (double)x.Quantity);
...
WITH ventas12 AS (
    ...
    WHERE (v.""IsDeleted"" = false OR v.""IsDeleted"" IS NULL)
      AND v.""Date"" >= @Desde12
...
SELECT p.""Id"",
       p.""Name"",
       c.""Name"" AS Categoria,
       COALESCE(v12.ventas12, 0) AS Ventas12m,
       CAST(p.""Stock"" AS DOUBLE PRECISION) AS StockActual,
       uv.ultima AS UltimaVenta,
       COALESCE(v3.ventas3, 0) AS Ventas3,
       COALESCE(vp3.ventasPrev3, 0) AS VentasPrev3,
       p.""Price"",
       p.""PrecioCosto""
FROM ""Productos"" p
...
";
            ...
            AddParam("@TenantId", _db.TenantId);
            ...
            using (var reader = await command.ExecuteReaderAsync())
            {
                ...
                {
                    var stock = reader.IsDBNull(4) ? 0 : Convert.ToDecimal(reader.GetValue(4));
                    var ventas12 = reader.IsDBNull(3) ? 0 : Convert.ToInt64(reader.GetValue(3));
                    var price = reader.IsDBNull(8) ? 0 : Convert.ToDecimal(reader.GetValue(8));
                    var costo = reader.IsDBNull(9) ? 0 : Convert.ToDecimal(reader.GetValue(9));
                    ...
                    var ventas3 = reader.IsDBNull(6) ? 0 : Convert.ToInt64(reader.GetValue(6));
                    var ventasPrev3 = reader.IsDBNull(7) ? 0 : Convert.ToInt64(reader.GetValue(7));
                    string tendencia = "→";
        ...
        /// <summary>
        /// En PostgreSQL con EF Core Migrations, las columnas siempre existen.
        /// Este método existe solo por compatibilidad con código heredado.
        /// </summary>
        private Task EnsureSoftDeleteColumnsAsync() => Task.CompletedTask;


        // Método heredado de SQLite — no necesario en PostgreSQL
        private Task EnsureColumnAsync(string table, string column, string definitionSql) => Task.CompletedTask;
    }
}
